...
const Employer = require("../db/Employer");
...
// to get all the jobs for employer personal and for everyone
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  // to list down jobs posted by a particular employer
  ...
  User.findOne({ _id: req.params.id })
    .then((userData) => {
      ...
      if (userData.type === "recruiter") {
        Employer.findOne({ userId: userData._id })
          .then((recruiter) => {
            if (recruiter === null) {
              res.status(404).json({
                message: "User does not exist",
        ...
        JobSeeker.findOne({ userId: userData._id })
          .then((jobApplicant) => {
            if (jobApplicant === null) {
              res.status(404).json({
                message: "User does not exist",
...
// employer/applicant gets all his applications
...
// get a list of final applicants for current job : employer
// get a list of final applicants for all his jobs : employer
